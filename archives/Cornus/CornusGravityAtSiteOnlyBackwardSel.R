...
require(nlme)
load("Cornus.data.rda")

# Saturated
# Backward Selection

sat.data <- cornus.data
sat.data$group <- as.factor(sat.data$Grouping)
...
gravity.1 <- gravity(y="GDist", x=c("DBH","PctSky","Clumping","FloralOutput","PDist"),
                     group="group", data=sat.data)
print(gravity.1$formula)
print(lme.sum(gravity.1$GravityModel))
AIC <- c(AIC,gravity.1$AIC)
r2 <- c(r2, find.r2(gravity.1$GravityModel))

gravity.2 <- gravity(y="GDist", x=c("DBH","PctSky","Clumping","FloralOutput"),
                     group="group", data=sat.data)
print(gravity.2$formula)
print(lme.sum(gravity.2$GravityModel))
AIC <- c(AIC,gravity.2$AIC)
r2 <- c(r2, find.r2(gravity.2$GravityModel))

gravity.3 <- gravity(y="GDist", x=c("DBH","Clumping","FloralOutput"),
                     group="group", data=sat.data)
print(gravity.3$formula)
print(lme.sum(gravity.3$GravityModel))
AIC <- c(AIC,gravity.3$AIC)
r2 <- c(r2, find.r2(gravity.3$GravityModel))

gravity.4 <- gravity(y="GDist", x=c("Clumping","FloralOutput"),
                     group="group", data=sat.data)
print(gravity.4$formula)
print(lme.sum(gravity.4$GravityModel))
AIC <- c(AIC,gravity.4$AIC)
r2 <- c(r2, find.r2(gravity.4$GravityModel))

gravity.5 <- gravity(y="GDist", x=c("FloralOutput"),
                     group="group", data=sat.data)
print(gravity.5$formula)
print(lme.sum(gravity.5$GravityModel))
AIC <- c(AIC,gravity.5$AIC)
r2 <- c(r2, find.r2(gravity.5$GravityModel))
...
graph.data <- cornus.data[ cornus.data$InGraph, ]
...
gravity.2 <- gravity(y="GDist", x=c("DBH","Clumping","FloralOutput","PDist"),
                     group="group", data=graph.data)
print(gravity.2$formula)
print(lme.sum(gravity.2$GravityModel))
AIC <- c(AIC,gravity.2$AIC)
...
gravity.3 <- gravity(y="GDist", x=c("Clumping","FloralOutput","PDist"),
                     group="group", data=graph.data)
print(gravity.3$formula)
print(lme.sum(gravity.3$GravityModel))
AIC <- c(AIC,gravity.3$AIC)
