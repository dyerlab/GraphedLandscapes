...
#                                                   #
#####################################################
tmp <- unique( cbind( cornus.off$Mom, cornus.off$X, cornus.off$Y ) )
...
for(i in 1:K){
  for(j in i:K){
    if(j>i){
      ...
      vals.cornus <- extract(cornus.raster, spLines )[[1]]
      vals.roads <- extract(roads.raster, spLines )[[1]]
      
      vals.open <- extract(open.raster,spLines )[[1]]
      vals.pines <- extract(pines.raster, spLines )[[1]]
